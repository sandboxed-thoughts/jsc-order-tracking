...
from ..models import Builder, Subdivision, Lot
...
class BuilderAdmin(SHA):
    list_display = ('name', 'get_addr', 'phone','email','is_active','get_history',)
...
class SubdivisionAdmin(SHA):
    list_display = ('name', 'get_addr','is_active','get_history',)
    list_filter = ('city','is_active',)
    fieldsets = (
        (None, {
            "fields": (
...
# not registered
class LotAdmin(SHA):
    list_display = ('name', 'builder','subdivision','get_history',)
    list_filter = ('builder','subdivision')
    fieldsets = (
        (None, {
            "fields": (
                ('name','builder','subdivision',),
            ),
        }),)
    actions = [activate,deactivate,]
