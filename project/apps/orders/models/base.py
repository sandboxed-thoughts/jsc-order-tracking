...
class BaseOrder(models.Model):
    ...
    job_site = models.ForeignKey(
        "jobs.JobSite", verbose_name=_("Job Site"), related_name="orders", on_delete=models.CharFieldCASCADE
    )
    ...
    qtype = models.CharField(
        _("Quantity Type"), max_length=4, choices=QTypeChoices.choices, default=QTypeChoices.LOADS
    )
    ...
    temp = models.CharField(_("Temperature"), max_length=4, choices=TempChoices.choices, default=TempChoices.AVERAGE)
...
class OrderNotes(models.Model):
    order = models.ForeignKey(BaseOrder, verbose_name=_("Order"), related_name="order_notes", on_delete=models.CASCADE)
    note = models.TextField(_("order notes"), blank=True, null=True)
    history = HR(inherit=True)
