...
class TestGravelDeliverySchedule(TestCase):
    def setUp(self):
        ...
        self.order = baker.make("orders.GravelOrder")
        ...
        self.supplier_delivery = baker.make(
            GravelDeliverySchedule, supplier_delivers=True, driver=None, order=self.order
        )

    def test_get_driver_for_supplier_delivers(self):
        self.assertEqual(self.supplier_delivery.get_driver(), self.order.supplier.name)
